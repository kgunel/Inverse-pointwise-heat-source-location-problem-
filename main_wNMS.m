...
for heatSourceId=1:nHeatSource
    ...
    for trialId=1:maxnTrials
        fprintf('\n  Trial Id : %d\n',trialId);
        disp('------------------------------------------------------------------------------------------');
        [xmin,fmin,nFeval,nExp,nIC,nOC,nShrink,iter,BestCost] = wANMS('Cost', 2, low, up, Max_Iteration, maxnFeval, tol, sol(heatSourceId).u, sol(heatSourceId).x, sol(heatSourceId).y, varEps);
...
%title('\bf Randomly generated locations of 10 heat sources founded by wANMS','Interpreter','Latex');
...
hold off
fig=gcf;
print(fig,'HeatSources_wANMS.eps','-painters','-depsc','-r300');
